...
#Nome: Andreza Serra - comp5 - Tópicos Especiais I

#Mineração de Textos para descoberta de padrões frequentes - Lteratura Biomédica - COVID19
...
#Convertendo o dataframe pmids para uma lista de inteiros
pmids = pmids$V1
...
df_achados_disease_id = df_achados_disease_id[order(df_achados_disease_id$Freq, 
                                                    decreasing = TRUE),]
df_achados_disease_id$Lista_achados_disease_id = as.character(
                                df_achados_disease_id$Lista_achados_disease_id)

#gráfico de barras
barplot(df_achados_disease_id$Freq[1:30],  col = rainbow(10), 
        legend = df_achados_disease_id$Lista_achados_disease_id[1:30], 
        legend.text = df_achados_disease_id$Lista_achados_disease_id[1:30])

# ------------------- EXTRAINDO FREQUÊNCIAS DE QUÍMICOS PELO ID----------------------------
Lista_achados_chemical_id = unlist(Lista_achados_chemical_id)
Lista_achados_chemical_id = tolower(Lista_achados_chemical_id)
Lista_achados_chemical_id = as.data.frame(table(Lista_achados_chemical_id))
Lista_achados_chemical_id = Lista_achados_chemical_id[order(Lista_achados_chemical_id$Freq, 
                                                            decreasing = TRUE),]

#gráfico de barras
barplot(Lista_achados_chemical_id$Freq[1:30],  col = rainbow(10), 
        legend = Lista_achados_chemical_id$Lista_achados_chemical_id[1:30], 
        legend.text = Lista_achados_chemical_id$Lista_achados_chemical_id[1:30])

# ------------------ EXTRAINDO FREQUÊNCIAS DE GENES PELO ID --------------------------
Lista_achados_gene_id = unlist(Lista_achados_gene_id)
Lista_achados_gene_id = toupper(Lista_achados_gene_id)
Lista_achados_gene_id = as.data.frame(table(Lista_achados_gene_id))
Lista_achados_gene_id = Lista_achados_gene_id[order(Lista_achados_gene_id$Freq, 
                                              decreasing = TRUE),]
#gráfico de barras
barplot(Lista_achados_gene_id$Freq[1:30],  col = rainbow(10), 
        legend = Lista_achados_gene_id$Lista_achados_gene_id[1:30], 
        legend.text = Lista_achados_gene_id$Lista_achados_gene_id[1:30])

#######################################################################################################
#------------------------- MINERAÇÃO DE ITENS FREQUENTES --------------------------------------

#EXPRESSÃO REGULAR PARA ENCONTRAR FRASES DE CONCLUSÃO/RESULTADO
reg_expression_conclusion = 
  '(results|findings|conclusion|data|taken|together|study|studies)
[\\w]*(.*)(suggest|indicate|suggested|indicated|illustrated|illustrate|revealed)[\\w]*'

#LAÇO DE REPETIÇÃO PARA A TOKENIZAÇÃO DOS RESUMOS EM FRASES, 
# COLETANDO POSIÇÃO DA FRASE NO RESUMO E OUTRAS INFORMAÇÕES
...
for(i in 1:nrow(df_mining)){
  df_mining$sentence_original[i] = paste(unlist(
    strsplit(df_mining$sentence_original[i], " ")), collapse = ',')
}
...
#MINERAÇÃO DE REGRAS DE ASSOCIAÇÃO ALGORITMO APRIORI, SUPORTE 0.001 E MÍNIMO DE 2 ITENS
